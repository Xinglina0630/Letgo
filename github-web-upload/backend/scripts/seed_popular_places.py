...
def destination_cities() -> list[str]:
    return [item["city"] for item in load_destinations()]


def seed(city_filter: str = "", offset: int = 0, city_limit: int = 0) -> tuple[int, int, int]:
    ...
    if city_filter:
        ...
        destinations = destinations[max(0, offset):max(0, offset) + city_limit]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--offset", type=int, default=0, help="Start city index for batch import")
    parser.add_argument("--limit", type=int, default=0, help="Number of cities in this batch")
    args = parser.parse_args()
    added, existing, failed = seed(args.city, args.offset, args.limit)
