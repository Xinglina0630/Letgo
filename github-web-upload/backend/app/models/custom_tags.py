...
class TravelProjectCustomTag(Base):
    ...
    id = Column(String(32), primary_key=True, default=_new_id)
    project_id = Column(String(32), ForeignKey("travel_projects.id"), nullable=False, index=True)
    custom_tag_id = Column(String(32), ForeignKey("custom_place_tags.id"), nullable=False, index=True)
    shared_by_user_id = Column(String(32), ForeignKey("users.id"), nullable=False)
    created_at = Column(DateTime, default=datetime.utcnow, nullable=False)
