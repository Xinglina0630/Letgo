...
from app.models.collaboration import (
    ...
    gen_invite_token, gen_invite_code, hash_token,
)

router = APIRouter(prefix="/api/itineraries", tags=["itinerary"])


def _broadcast_itinerary(project_id: str, event_type: str, user: User, itinerary: Itinerary) -> None:
    async def _send():
        try:
            ...
            await broadcast_project_event(project_id, {
                ...
                "summary": f"更新了攻略「{itinerary.name}」",
            })
        except Exception:
            pass
...
async def update_itinerary(
    ...
    old_node_count = len(itinerary.nodes or [])
    try:
    ...
    if updated.project_id:
        # Use the submitted replacement list instead of the ORM relationship.
        # Bulk replacement can leave an already-loaded relationship stale in
        # this request, which used to misclassify deletions as normal updates.
        new_node_count = len(data.nodes or [])
        event_type = "itinerary_nodes_deleted" if new_node_count < old_node_count else "itinerary_updated"
        db.add(TravelProjectEvent(
            ...
            summary=f"更新了攻略「{updated.name}」",
        ))
        db.commit()
        _broadcast_itinerary(updated.project_id, event_type, user, updated)
    return updated
